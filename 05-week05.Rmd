...
In the first part of this week's session we will start by creating a **tidy dataset** using some data from the [UK 2011 Census of Population](https://www.ons.gov.uk/census/2011census). In the second part of this week's session we will explore potential factors that may contribute to bicycle theft in London. We conduct an ESDA of these variables, followed by statistical and spatial regression analysis to determine whether our variables contribute to bicyle theft in our area of study. 
...
# load spatial data
lsoa <- st_read('raw/boundaries/london_lsoa_2011/lsoa_london_2011.shp')
...
This is all looking fine. Let's now turn our turn our bike theft location dataset into a spatial dataset using the longitude and latitude values and get total bike theft counts within each of London's LSOAs.
...
> Please note: the following operation requires some computing power as for all of our LSOAs in London we are going to calculate the euclidean distance from its centroid to all tube stations and create a **distance matrix**. In case the calculation takes too much time on your computer, you can also download the [distance matrix](https://github.com/jtvandijk/GEOG0114/tree/master/raw/zip/distance_matrix.zip) and use `dist_matrix <- data.matrix(read_csv('path_to_file'))` to continue.
...
# gamblinc outlets distance vs bike theft 
ggplot(lsoa_join,aes(x=gamb_dist, y=n_crime_cube)) +
  geom_point()
