...
names(lsoa) <- c('lsoa11cd','popdens','geometry')
...
This is all looking fine. Let's now turn our turn our crime location dataset into a spatial dataset using the longitude and latitude values and get total crime counts within each of London's LSOAs.
...
# summary
summary(lsoa_join$n_crime_sqrt)
...
# imd vs crime
ggplot(lsoa_join,aes(x=imd_dec, y=n_crime_sqrt)) +
  geom_point()
...
# pub distance vs crime
ggplot(lsoa_join,aes(x=pubs_dist, y=n_crime_sqrt)) +
  geom_point()

# off license distance vs crime
ggplot(lsoa_join,aes(x=off_dist, y=n_crime_sqrt)) +
  geom_point()
```
...
# imd vs crime
cor(lsoa_join$imd_dec,lsoa_join$n_crime_sqrt)

# gambling outlet distance vs crime
cor(lsoa_join$gamb_dist,lsoa_join$n_crime_sqrt)

# pub distance vs crime
cor(lsoa_join$pubs_dist,lsoa_join$n_crime_sqrt)

# off license distance vs crime
cor(lsoa_join$off_dist,lsoa_join$n_crime_sqrt)
```
So, looking at these graphs there seems to be some relationship between our explantory (independent) variables and our dependent variables. It is time to form a hypothesis: LSOAs that are in a more deprived IMD decile and LSOAs with a lower mean distance to one of our outlets of interest experience more anti-social behaviour. The null hypothesis then becomes: LSOAs that are in a more deprived IMD decile and LSOAs with a lower mean distance to one of our outlets of interest **do not** experience more anti-social behaviour, i.e. there is no relationship whatsoever.
