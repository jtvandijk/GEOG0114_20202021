...
camden_oas <- st_read("raw/administrative_boundaries/camden_oas_2011/OAs_camden_2011.shp", 
                      crs=27700)
...
``` {r 04-e-shared-boundary-neighbour-function-creation, echo=FALSE}
...
Now the sum of one row instead of being the total number of neighbours for that OA instead adds up to 1. This is important for the calculation of Moran's I.
...
camden_oas_moran_stats$quad_sig <- ifelse(camden_oas_moran_stats$scale_n_thefts > 0 & 
                                          camden_oas_moran_stats$scale_n_thefts > 0 & 
                                          local_moran_camden_oa_theft[,5] <=0.05, 
                                          "high-high", 
                                   ...
                                   ifelse(camden_oas_moran_stats$scale_n_thefts <= 0 & 
                                          ...
                                          local_moran_camden_oa_theft[,5] <= 0.05, 
                                          "low-high",
                                   ifelse(local_moran_camden_oa_theft[,5] > 0.05, 
                                          "not-significant", 
                                          "not-significant")))))
...
Now that that horrible looking conditional over we can map the results!
...
As our data are so spatially clustered we can't see any outlier places. This suggests that the pattern of theft from persons is not highly concentrated in very small areas or particular Output Areas, and instead is spread on a larger scale than we have used here.
